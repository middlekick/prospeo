...
export default function RdvCalendar({ leads }: Props) {
  ...
  // Construction de la grille du mois (semaines commenÃ§ant lundi)
  const grid = useMemo(() => {
  ...
  return (
    <div className="rounded-2xl border border-white/[0.10] bg-white/[0.06] p-5">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-2">
          <button onClick={prevMonth}
            className="w-6 h-6 rounded-lg bg-white/[0.05] hover:bg-white/[0.1] text-slate-500 hover:text-slate-300 text-xs transition-all">â€¹</button>
          <span className="text-xs font-medium text-slate-300 w-32 text-center">
            {MONTHS[cursor.m]} {cursor.y}
          </span>
          <button onClick={nextMonth}
            className="w-6 h-6 rounded-lg bg-white/[0.05] hover:bg-white/[0.1] text-slate-500 hover:text-slate-300 text-xs transition-all">â€º</button>
        </div>
      </div>
          ...
          return (
            <button
              ...
              className={[
                ...
                isSelected   ? "bg-brand-500/25 ring-1 ring-brand-500/40"
              ...
              {rdvs.length > 0 && (
                <div className="flex gap-0.5">
                  {rdvs.slice(0, 3).map((r, j) => (
                    <span key={j} className={`w-1 h-1 rounded-full ${RDV_DOT[r.rdv_statut] || "bg-brand-400"}`} />
                  ))}
                </div>
              )}
            </button>
          ...
          {selectedRdv
            ...
            .map((l, i) => (
              <div key={i} className="flex items-center gap-3 px-3 py-2 rounded-lg bg-white/[0.03] border border-white/[0.05]">
                {l.rdv_heure && (
                  <span className="text-xs mono text-brand-300 shrink-0 w-12">{l.rdv_heure}</span>
                )}
                <div className="flex-1 min-w-0">
                  <div className="text-sm text-slate-200 truncate">{l.nom}</div>
                  <div className="text-xs text-slate-600 truncate">{l.metier}{l.rdv_lieu ? ` Â· ${l.rdv_lieu}` : ""}</div>
                </div>
                <span className={`w-2 h-2 rounded-full shrink-0 ${RDV_DOT[l.rdv_statut] || "bg-brand-400"}`} />
              </div>
            ))}
        </div>
      )}
      ...
      <div className="flex items-center gap-3 mt-4 pt-3 border-t border-white/[0.06] text-[10px] text-slate-700">
        <span className="flex items-center gap-1"><span className="w-1.5 h-1.5 rounded-full bg-green-400" />ConfirmÃ©</span>
        <span className="flex items-center gap-1"><span className="w-1.5 h-1.5 rounded-full bg-yellow-400" />En attente</span>
        <span className="flex items-center gap-1"><span className="w-1.5 h-1.5 rounded-full bg-slate-500" />EffectuÃ©</span>
      </div>
    </div>
  );
}
