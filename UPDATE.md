...
| 2026-05-18 | Hardening | **Garde anti-mojibake durable** : `.editorconfig` + `.gitattributes` (UTF-8/LF) + `scripts/check-encoding.js` + hook pre-commit auto-installé (`npm run prepare`) → refuse tout octet 0x80-0x9F/BOM au commit | `.editorconfig`, `.gitattributes`, `scripts/`, `package.json` |
| 2026-05-18 | Hardening | **Rate limiting** (fenêtre glissante en mémoire, `lib/rate-limit.ts`) sur `/api/scrape` (12/5min), `/api/contact` (5/10min), `/api/trial` (10/10min) par IP → anti-abus/spam | `lib/rate-limit.ts`, 3 routes |
| 2026-05-18 | SEO | Image OG dynamique `app/opengraph-image.tsx` (next/og, 1200×630, brandée cyan) → aperçus de partage LinkedIn/X | `app/opengraph-image.tsx` |
| 2026-05-18 | Design | Emoji → SVG dans le LeadDrawer (journal d'activité 🔄📧💬📞, sélecteur email 🚀✅⏰, cadenas, boutons RDV) — conforme au design system SVG. Reste : CallSession/KanbanView/OnboardingModal (cosmétique, non bloquant) | `components/leads/LeadDrawer.tsx` |
