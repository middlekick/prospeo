      ...
      <div className="bg-[#13151e] border border-white/[0.08] rounded-xl p-5 max-w-md w-full space-y-3">
        ...
        {clerkUser?.id && (
          <div className="flex items-center gap-2">
            <code className="flex-1 text-xs mono bg-white/[0.06] border border-white/[0.08] rounded-lg px-3 py-2 text-brand-300 break-all">
              {clerkUser.id}
            </code>
            <button onClick={() => navigator.clipboard.writeText(clerkUser.id)}
              className="h-8 px-3 rounded-lg bg-brand-500/15 border border-brand-500/25 text-brand-400 text-xs hover:bg-brand-500/25 transition-all shrink-0">
              Copier
            </button>
          </div>
        )}
            ...
            {(["all","free","trial","pro","agency"] as const).map(f => (
              <button key={f} onClick={() => setFilter(f)}
                className={[
                  ...
                  filter === f ? "bg-brand-500/20 text-brand-200" : "text-slate-600 hover:text-slate-300",
                ].join(" ")}>
                    ...
                    <td className="py-3 px-4">
                      <div className="flex items-center gap-2.5">
                        {user.clerk?.imageUrl
                          ...
                          : <div className="w-7 h-7 rounded-full bg-brand-500/20 flex items-center justify-center text-xs text-brand-300 shrink-0 font-semibold">
                              ...
                              {Object.entries(user.tags).sort((a,b) => b[1]-a[1]).map(([tag, count]) => (
                                <div key={tag} className="flex items-center gap-2">
                                  ...
                                  <div className="flex-1 h-1 bg-white/[0.05] rounded-full overflow-hidden">
                                    <div className="h-full bg-brand-500/40 rounded-full"
                              ...
                              <div className="flex items-center gap-2">
                                ...
                                {["pro", "agency", "free"].map(p => (
                                  <button key={p} onClick={() => setPlan(user.user_id, p)}
                                    ...
                                    className={`h-6 px-2 rounded-md text-[10px] font-medium transition-all disabled:opacity-30 ${
                                      p === "pro"    ? "bg-brand-500/10 border border-brand-500/20 text-brand-400 hover:bg-brand-500/20" :
